/* fl_Preproc.c — Flame препроцессор с полным набором возможностей C-препроцессора
 *
 * Поддерживаемые директивы:
 *   #include "file" / <file>
 *   #define NAME [value]
 *   #define NAME(args...) body        — функциональные макросы
 *   #define NAME(args..., ...) body   — variadic макросы (__VA_ARGS__)
 *   #undef NAME
 *   #if <const-expr>                  — поддержка defined(), арифметики, сравнений
 *   #ifdef / #ifndef
 *   #elif <const-expr>
 *   #else
 *   #endif
 ...
 *   #pragma once
 *   #line <num> ["file"]
 *   #import "file"                    — Flame-специфичная директива
 *
 * Операторы в макросах:
 *   #arg   — stringify
 *   a ## b — token pasting
 *
 * Предопределённые макросы:
 *   __FILE__, __LINE__, __DATE__, __TIME__, __COUNTER__,
 *   __FLAME__, __FLAME_VERSION__
 */
...
#define MAX_EXPAND_DEPTH 64   /* защита от бесконечной рекурсии */
...
/* Описание макроса (объектный или функциональный) */
typedef struct {
    char  name[64];
    char  value[512];        /* тело макроса */
    char  params[MAX_MACRO_PARAMS][64]; /* имена параметров */
    int   param_count;       /* -1 = объектный макрос */
    int   is_variadic;       /* 1 если есть ... */
} Define;
...
typedef struct {
    ...
    char (*included_files)[512]; /* файлы с include guard */
    size_t included_count;
    size_t max_includes;

    char (*pragma_once_files)[512]; /* файлы с #pragma once */
    size_t pragma_once_count;
    size_t max_pragma_once;

    /* текущий файл/строка для __FILE__ и __LINE__ */
...
static void mark_pragma_once(PreprocCtx *ctx, const char *canon_path) {
    if (ctx->pragma_once_count >= ctx->max_pragma_once) {
        ...
        if (!tmp) { fprintf(stderr, "preprocessor: out of memory\n"); exit(1); }
...
static void define_push(PreprocCtx *ctx, const char *name, const char *value,
    ...
    /* Перезаписываем если уже определён */
    ...
    if (ctx->define_count >= ctx->max_defines) {
        ...
        if (!tmp) { fprintf(stderr, "preprocessor: define table overflow\n"); exit(1); }
        ctx->defines = tmp;
    }
...
static char *read_file(const char *path) {
    FILE *f = fopen(path, "r");
    if (!f) { fprintf(stderr, "preprocessor: cannot open '%s'\n", path); return NULL; }
    fseek(f, 0, SEEK_END);
    long size = ftell(f);
    rewind(f);
...
/* Разбор аргументов вызова макроса: FOO(a, b, c)
 * Возвращает число аргументов, заполняет args[] */
...
/* Подстановка параметров в тело макроса, обработка # и ## */
static char *substitute_params(const char *body, char params[][64], char args[][512],
    ...
    while (bi < blen && ri < (int)sizeof(result) - 1) {
        ...
        if (body[bi] == '#' && bi + 1 < blen && body[bi + 1] != '#') {
            ...
            /* ищем параметр */
            const char *arg_val = pname; /* fallback */
            ...
            /* stringify: оборачиваем в кавычки */
            result[ri++] = '"';
        ...
        /* идентификатор — проверяем параметры */
...
/* Основная функция раскрытия макросов в строке text */
static char *expand_macros(PreprocCtx *ctx, const char *text, int depth) {
...
#define ROUT(ch) do { \
    ...
    while (ti < tlen) {
        ...
        if (isalpha((unsigned char)text[ti]) || text[ti] == '_') {
            ...
            /* Функциональный макрос — нужны скобки */
            int saved_ti = ti;
            while (ti < tlen && (text[ti] == ' ' || text[ti] == '\t')) ti++;
            if (ti >= tlen || text[ti] != '(') {
                /* нет скобок — не раскрываем */
                memcpy(result + ri, word, wi);
                ri += wi;
                if ((size_t)ri >= result_cap - 1) {
                    result_cap *= 2;
            ...
            /* variadic: последние аргументы идут в __VA_ARGS__ */
...
/* ═══════════════════════════════════════════════════════════
 *  Вычисление константных выражений (#if)
 * ═══════════════════════════════════════════════════════════ */
...
static long long eval_primary(PreprocCtx *ctx, ExprParser *ep) {
    ...
    /* Числа (десятичные, шестнадцатеричные) */
...
static long long eval_if_expr(PreprocCtx *ctx, const char *expr) {
    /* Сначала раскрываем макросы в выражении */
...
/* ═══════════════════════════════════════════════════════════
 *  Вспомогательные: push нового if-фрейма
 * ═══════════════════════════════════════════════════════════ */
static void if_push(PreprocCtx *ctx, int cond) {
    if (ctx->if_depth >= ctx->max_if_depth) {
        ...
        if (!tmp) { fprintf(stderr, "preprocessor: out of memory in #if stack\n"); exit(1); }
        ctx->if_stack = tmp;
    }
...
#define OUT_PUSH(ch) do { \
    if (ctx->out_i >= ctx->output_size - 1) { \
        ...
        if (!re_out) { fprintf(stderr, "preprocessor: output buffer overflow\n"); exit(1); } \
        ctx->out = re_out; \
    } \
...
#define STR_PUSH(buf, idx, maxlen, ch) do { \
    ...
    /* Буфер для текущей строки (для раскрытия макросов построчно) */

    while (src_i < src_len) {

        /* ── однострочный комментарий ── */
        if (source[src_i] == '/' && src_i + 1 < src_len && source[src_i+1] == '/') {
            /* передаём в вывод как есть (лексер сам уберёт) */
        ...
        if (source[src_i] == '#') {
            ...
            /* ── #elif ── */
                ...
                expr[ei] = '\0';
                if (ctx->if_depth == 0) {
                    fprintf(stderr, "preprocessor: #elif without #if at line %d\n", ctx->current_line);
                } else {
            ...
            /* ── #else ── */
                ...
                if (ctx->if_depth == 0) {
                    fprintf(stderr, "preprocessor: #else without #if at line %d\n", ctx->current_line);
                } else {
                    IfFrame *fr = &ctx->if_stack[ctx->if_depth - 1];
            ...
            /* ── #endif ── */
                ...
                if (ctx->if_depth == 0)
                    fprintf(stderr, "preprocessor: #endif without #if at line %d\n", ctx->current_line);
                else
                    ctx->if_depth--;
            }
            /* ── #define ── */
                ...
                int  param_count = -1; /* -1 = объектный */
                int  is_variadic = 0;

                /* Функциональный макрос: имя сразу за '(' без пробела */
                ...
                /* Тело: может быть многострочным с \ */
                char value[512] = {0}; int vi = 0;
                while (src_i < src_len && source[src_i] != '\n') {
                    if (source[src_i] == '\\' && src_i + 1 < src_len && source[src_i+1] == '\n') {
                        src_i += 2;
                        ctx->current_line++;
                        /* пробел вместо переноса */
            ...
            /* ── #error ── */
                ...
                msg[mi] = '\0';
                if (currently_active(ctx)) {
                    fprintf(stderr, "preprocessor: #error at line %d: %s\n", ctx->current_line, msg);
                    return -1;
                }
            }
            /* ── #warning ── */
                ...
                msg[mi] = '\0';
                if (currently_active(ctx))
                    fprintf(stderr, "preprocessor: warning at line %d: %s\n", ctx->current_line, msg);
            }
            /* ── #line ── */
            ...
            /* ── #pragma ── */
                ...
                if (currently_active(ctx) && strcmp(pragma_arg, "once") == 0) {
                    /* Помечаем текущий файл как уже включённый через pragma once */
                ...
                /* Прочие #pragma — молча игнорируем */
            }
            else {
                if (currently_active(ctx))
                    fprintf(stderr, "preprocessor: unknown directive '#%s' at line %d\n",
        ...
        /* ── неактивная ветка — пропускаем всё ── */
        if (!currently_active(ctx)) {
            if (source[src_i] == '\n') ctx->current_line++;
            src_i++;
            continue;
        ...
        /* ── идентификатор — раскрытие макросов ── */
        if (isalpha((unsigned char)source[src_i]) || source[src_i] == '_') {
            ...
            Define *def = define_lookup(ctx, word);
            if (!def) {
                /* Не макрос — выводим слово как есть */
                memcpy(ctx->out + ctx->out_i, word, wi);
                ctx->out_i += wi;
                if (ctx->out_i >= ctx->output_size - 1) {
                    ctx->output_size *= 2;
                    char *tmp = realloc(ctx->out, ctx->output_size);
                    if (!tmp) { fprintf(stderr, "preprocessor: output buffer overflow\n"); exit(1); }
                    ctx->out = tmp;
                }
                continue;
            }
            ...
            /* Функциональный макрос — ищем '(' */
            while (src_i < src_len && (source[src_i] == ' ' || source[src_i] == '\t')) src_i++;
            if (src_i >= src_len || source[src_i] != '(') {
                /* скобок нет — оставляем как есть */
                memcpy(ctx->out + ctx->out_i, word, wi);
                ctx->out_i += wi;
                if (ctx->out_i >= ctx->output_size - 1) {
                    ctx->output_size *= 2;
            ...
            /* Читаем строку для разбора аргументов (поддержка переносов строк внутри) */
            /* Собираем всё от '(' до закрывающей ')' */
            ...
            /* parse_macro_args ожидает строку начинающуюся с '(' */
            char args[MAX_MACRO_PARAMS + 8][512];
            memset(args, 0, sizeof(args));
            size_t pos2 = 0;
...
char *preprocess(const char *source, const char *base_dir) {
    /* Ленивая инициализация глобальных массивов */
    if (!g_import_objects) {
    ...
    if (base_dir) strncpy(ctx.current_file, base_dir, sizeof(ctx.current_file) - 1);
    ctx.current_line = 1;

    /* Предопределённые объектные макросы */
    ...
    if (ctx.if_depth > 0) {
        fprintf(stderr, "preprocessor: unclosed #if/#ifdef/#ifndef (%zu levels)\n", ctx.if_depth);
        rc = -1;
    }
